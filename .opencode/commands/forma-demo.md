...
Use the local `forma` MCP server for this demo. Create a low-voltage hardware project with useful mechanical dimensions and component placements for a room-scale Astra scene.
...
3. Save the compiled manifest as `demo/forma-project.json` in the Astra checkout. Keep the manifest self-contained where possible and do not include credentials, tokens, prompts containing secrets, or server logs.
