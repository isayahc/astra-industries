...
export async function connectCloudStorage(expectedOwner: string): Promise<CloudStorage> {
  if (!cloudStorageEnabled || !supabase) throw new Error('Cloud storage is disabled. The local library is still available.');
  ...
  return new CloudStorage(client, expectedOwner);
}
