...
export function SceneControls({workspace,owner,roomId,setRoomId,current,setCurrent,replace,busy,setBusy,onExport}: {
  ...
  function openScene(scene:SavedScene){
    ...
    void run(async(repo,token)=>{const local=await listLibrary();const result=await repo.open(scene.id,[...workspace.items.filter(i=>!i.missing).map(i=>i.asset),...local.map(e=>e.asset)],text=>{if(valid(token))setMessage(text);});if(!valid(token))return;replace(result.workspace);setCurrent(result.scene);setRoomId(result.scene.id);setName(result.scene.name);setMessage(result.workspace.items.some(i=>i.missing)?'Scene opened with missing geometry. Reimport matching source files.':'Cloud scene opened.');});
  }
  ...
  return <section aria-label="Scene persistence">
    ...
    {!owner?<p>Sign in to save scenes and library metadata across devices. You can export/import scene JSON locally.</p>:<>
      ...
      <div className="capture-actions"><button disabled={busy||!name.trim()} onClick={()=>void run(async(repo,token)=>{
        ...
        if(!valid(token))return;replace(result.workspace);setCurrent(result.scene);setRoomId(result.scene.id);setMessage('Scene saved to Postgres.');setScenes(await repo.list());
      })}>Save cloud scene</button><button disabled={busy} onClick={()=>{if(!confirmSwitch('Create a new room'))return;replace(emptyWorkspace());setCurrent(null);setRoomId(crypto.randomUUID());setName('Untitled room');setMessage('New room ready.');}}>New room</button><button disabled={busy||!current} onClick={()=>{if(!confirmSwitch('Create a copy'))return;setCurrent(null);setRoomId(crypto.randomUUID());setName(`${name} copy`);setMessage('Copy ready to save as a new room.');}}>Save as new copy</button><button disabled={busy} onClick={()=>void run(async(repo,token)=>{const rows=await repo.list();if(valid(token))setScenes(rows);})}>Refresh scenes</button></div>
