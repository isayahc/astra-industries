...
function App() {
  ...
  const project = asset?.formaProject;
  const projectValue = (value: unknown) => JSON.stringify(value, null, 2);
  ...
  return <><header className="app-header"><div className="brand"><span className="logo">A</span> ASTRA <span className="muted">INDUSTRIES</span></div><span className="tag">SPATIAL WORKBENCH / 001</span><AuthControls beforeSignIn={() => preserveAuthWorkspace(assets, room, selected, positions)} /><button disabled={busy} onClick={openFilePicker}>+ Import project</button></header>
    <main onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); void load(Array.from(e.dataTransfer.files)); }}>
     ...
     <aside className="inspector"><div className="eyebrow">INSPECTOR</div>{asset ? <><h2>{asset.name}</h2><span className="badge">{asset.source.kind.toUpperCase()}</span><h3>Dimensions</h3><p>{asset.dimensions.map(n => n.toFixed(3)).join(' × ')} m<br /><small>Width × height × depth</small></p><h3>Source</h3><p className="wrap">{asset.source.filename}<br />{asset.source.version && `Version ${asset.source.version}`}</p>{asset.warnings.map(w => <p className="notice" key={w}>{w}</p>)}{project && <><details open><summary>Forma project</summary><dl className="project-meta"><dt>Project ID</dt><dd>{project.projectId || 'Not provided'}</dd><dt>Revision</dt><dd>{project.revision || 'Not provided'}</dd><dt>Agent</dt><dd>{project.agent || 'Not provided'}</dd><dt>IR version</dt><dd>{project.hardwareIrVersion}</dd><dt>Input</dt><dd>{project.source}</dd></dl></details><details><summary>Overview</summary><pre>{projectValue(project.ir.overview || {})}</pre></details><details><summary>Validation</summary><pre>{projectValue(project.ir.validation || { status: 'No validation summary supplied' })}</pre></details><details><summary>Mechanical data</summary><pre>{projectValue(project.ir.mechanical || {})}</pre></details><details><summary>Bill of materials ({Array.isArray(project.ir.bom) ? project.ir.bom.length : 0})</summary><pre>{projectValue(project.ir.bom || [])}</pre></details></>}<h3>Components</h3>{asset.parts.map((part, partIndex) => <details key={part.id} open={partIndex === selectedPart}><summary><button className="part-select" onClick={() => { setSelected(selected); setSelectedPart(partIndex); }}>{part.name}</button></summary>{Object.entries(part.metadata).filter(([, v]) => v).map(([key, value]) => <p key={key}><b>{key}</b>: {value}</p>)}</details>)}</> : <><h2>A space for<br />what’s next.</h2><p>Select an imported asset to inspect its dimensions, parts, and source.</p><div className="room-stat">{room[0] * room[1]}<small>m² floor area</small></div></>}</aside>
